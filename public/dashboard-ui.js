...
function fallbackItems(snapshot = {}) {
  ...
  for (const container of snapshot.containers || []) if (!container.ok || ['unhealthy', 'starting'].includes(container.health)) items.push({ key: `container:${container.name}`, type: 'container-state', severity: !container.ok || container.health === 'unhealthy' ? 'critical' : 'warning', tab: 'system', title: `${container.name} needs attention`, source: 'Docker', advice: 'Controleer containerstatus en logs.', detail: 'containers', lastSeen: snapshot.generatedAt });
