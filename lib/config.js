...
export const defaults = {
  ...
  paths:{verifierLog:'/data/verifier.log',periodicLog:'/data/periodic-search.log',watchdogLog:'/data/watchdog.log',rcloneLog:'/data/rclone.log',bazarrLog:'/data/bazarr.log',actionDir:'/config/actions',actionLog:'/data/actions.log',tvRoot:'/media/tv',movieRoot:'/media/movies',nzbdavMount:'/nzbdav'},
...
export function validateConfig(value) {
  ...
  for(const [key,url] of Object.entries(value.links||{})){if(typeof url!=='string'||key.length<1||key.length>80||['__proto__','prototype','constructor'].includes(key)){errors.push(`invalid_url:links.${key}`);continue;}try{const u=new URL(url);if(!['http:','https:'].includes(u.protocol)||u.username||u.password)errors.push(`invalid_url:links.${key}`);}catch{errors.push(`invalid_url:links.${key}`);}}
  for(const [key,limit] of Object.entries(monitoringLimits)){const v=value.monitoring?.[key];if(typeof v==='number'&&v>limit)errors.push(`invalid_range:monitoring.${key}`);}
  return errors;
}
const monitoringLimits={sampleMinutes:1440,retentionDays:3650,resolvedHours:8760,queueStaleMinutes:10080,queueFailedMinutes:10080,mountWarnMs:60000,mountTimeoutMs:300000};
...
export function normalizeConfigInput(input){if(!safeObject(input))return{errors:['invalid_config'],value:{}};const errors=[];const value={};const top=new Set(['locale','managementMode','monitoring','paths','dockerSocket','containers','links',...integrations]);for(const key of Object.keys(input)){if(['__proto__','prototype','constructor'].includes(key)||!top.has(key))errors.push(`unknown_field:${key}`);}
 ...
 if('paths'in input){if(!safeObject(input.paths))errors.push('invalid_type:paths');else{value.paths={};for(const [key,v]of Object.entries(input.paths)){if(!pathFields.has(key))errors.push(`unknown_field:paths.${key}`);else if(typeof v!=='string'||v.length>2048||v.includes('\0'))errors.push(`invalid_type:paths.${key}`);else value.paths[key]=v;}}}
 if('links'in input){if(!safeObject(input.links)||Object.keys(input.links).length>32){errors.push('invalid_type:links');}else{value.links={};for(const [key,url] of Object.entries(input.links)){if(key.length<1||key.length>80||/[\u0000-\u001f\u007f]/.test(key)||['__proto__','prototype','constructor'].includes(key)||typeof url!=='string'||url.length>2048){errors.push(`invalid_type:links.${key}`);continue;}try{const u=new URL(url);if(!['http:','https:'].includes(u.protocol)||u.username||u.password)throw new Error('invalid_url');value.links[key]=url;}catch{errors.push(`invalid_url:links.${key}`);}}}}if('containers'in input){if(!Array.isArray(input.containers)||input.containers.some(x=>typeof x!=='string'||x.length>128))errors.push('invalid_type:containers');else value.containers=[...new Set(input.containers)];}return{errors,value};}
