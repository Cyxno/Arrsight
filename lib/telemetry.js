...
function normalizedPoint(point) {
  return {
    ...
    advice: point.advice || 'Check the affected service.', detail: point.detail || 'incidents',
...
export function buildAttentionItems(snapshot = {}, now = new Date()) {
  ...
  for (const container of snapshot.docker?.reachable === false ? [] : snapshot.containers || []) {
    ...
    if (stopped || unhealthy || starting) points.push(normalizedPoint({ key: `container:${container.name}`, type: 'container-state', severity: stopped || unhealthy ? 'critical' : 'warning', tab: 'system', title: `${container.name} ${stopped ? 'is stopped or missing' : unhealthy ? 'is unhealthy' : 'is still starting'}`, source: 'Docker', advice: 'Check container status and logs; restart only when safe.', detail: 'containers', lastSeen: container.checkedAt || now.toISOString() }));
    if (container.restartIncreased) points.push(normalizedPoint({ key: `container-restart:${container.name}:${container.restartCount}`, type: 'container-restart', severity: 'warning', tab: 'system', title: `${container.name} restarted unexpectedly`, source: 'Docker restart count', advice: 'Check container logs around the restart time.', detail: 'containers', lastSeen: container.checkedAt || now.toISOString() }));
  }
  if (snapshot.docker?.reachable === false) points.push(normalizedPoint({ key: 'docker:unreachable', type: 'docker-unreachable', severity: 'critical', tab: 'system', title: 'Docker is unreachable', source: 'Docker socket', advice: 'Check the Docker socket and dashboard permissions.', detail: 'containers', lastSeen: now.toISOString() }));
  for (const [name, health] of Object.entries(snapshot.health || {})) if (health?.reachable === false && !points.some((point) => point.type === 'api-unreachable' && point.title.toLowerCase().includes(name))) points.push(normalizedPoint({ key: `api:${name}`, type: 'api-unreachable', severity: 'critical', tab: 'system', title: `${name[0].toUpperCase() + name.slice(1)} API unreachable`, source: 'Live API-check', advice: 'Check the container, network, and API configuration.', detail: 'incidents', lastSeen: health.checkedAt || now.toISOString() }));
  for (const [name, check] of Object.entries(snapshot.mounts?.checks || {})) if (check.status === 'incident') points.push(normalizedPoint({ key: `mount:${name}`, type: 'mount-read-check', severity: 'critical', tab: 'system', title: `${name} mount not readable`, source: 'Live read-test', advice: 'Check rclone/InfiniDysk, then run the mount check.', detail: 'mount', lastSeen: check.checkedAt || now.toISOString() }));
  const verifier = snapshot.repairs?.summary || {};
  if (verifier.failed) points.push(normalizedPoint({ key:'verifier:failed', type:'verifier-failed', severity:'critical', tab:'downloads', title:'Verifier run failed', source:'Integrity verifier', advice:'Check the verifier log and fix the cause before starting another run.', detail:'repairs', lastSeen:now.toISOString() }));
  else if (verifier.incomplete) points.push(normalizedPoint({ key:'verifier:incomplete', type:'verifier-incomplete', severity:'warning', tab:'downloads', title:'Verifier run remained incomplete', source:'Integrity verifier', advice:'Check why the earlier verifier run did not finish cleanly.', detail:'repairs', lastSeen:now.toISOString() }));
...
export function verifierSummary(runs = [], now = new Date(), graceMinutes = 30) {
  const result = { runs: runs.length, succeeded: 0, clean: 0, failed: 0, incomplete: 0, running: 0, repairs: 0, period: `last ${runs.length} verifier-runs` };
...
export function reconcileIncidentHistory(previous = [], current = [], now = new Date(), resolvedWindowHours = 72, retentionDays = 90) {
  ...
  for (const item of current || []) {
    ...
    const newOccurrence = old && !old.active;
    map.set(key, { fingerprint: key, id: item.id, title: item.title, severity: item.severity, area: item.area, source: item.source, firstSeen: old?.firstSeen || item.firstSeen || at, lastSeen: item.lastSeen || at, resolvedAt: null, count: old ? Number(old.count || 1) + (newOccurrence ? 1 : 0) : Number(item.count || 1), active: true });
  }
...
function providerName(value, context='') {
  const text = String(value || '');
  if (!['provider-trip','provider-connection','missing-articles','provider-status'].includes(context)) return null;
  if (/\b(?:news\.)?vipernews\.com\b/i.test(text)) return 'Viper';
  if (/\b(?:news\.)?sunnyusenet\.[a-z]{2,}\b/i.test(text)) return 'Sunny';
  const expected=/\b(provider|usenet|news server|nntp|last error from)\b/i.test(text);
  if (!expected) return null;
  if (/\bviper\b/i.test(text)) return 'Viper';
  if (/\b(?:news\.)?sunny(?:usenet)?(?:\.[a-z]{2,})?\b/i.test(text)) return 'Sunny';
  const quoted = text.match(/(?:Provider|Last error from)\s+["']([^"']+)["']/i)?.[1];
  const host = text.match(/(?:provider|usenet|news server|nntp)[^\n]*?\b([a-z0-9-]+(?:\.[a-z0-9-]+)+)\b/i)?.[1];
  const plain = text.match(/Provider\s+([A-Za-z0-9._-]+)\s+(?:tripped|failed|timeout|connection)/i)?.[1];
  const candidate=(quoted||plain||host||'').replace(/[^A-Za-z0-9._ -]/g,'').slice(0,80);
  if(!candidate || /^\d+(?:\.\d+){1,3}$/.test(candidate) || /\.(mkv|mp4|avi|srt|nzb)$/i.test(candidate)) return null;
  return candidate;
}
...
export function deriveProviders(events = [], incidents = [], now = new Date(), windowHours = 24) {
  ...
  for (const event of events) {
    const name = providerName(`${event.subject || ''} ${event.line || ''}`,event.id); if (!name) continue;
  ...
  for (const incident of incidents) {
    const name = providerName(`${incident.subject || ''} ${incident.line || ''}`,incident.id); if (!name) continue;
    if (!providers.has(name)) providers.set(name, { name, trips: 0, missingArticles: 0, lastTrip: null, lastRecovery: null });
  }
  return [...providers.values()].map((provider) => {
    const active = incidents.filter((incident) => incident.active && providerName(`${incident.subject || ''} ${incident.line || ''}`,incident.id) === provider.name && ['provider-trip', 'provider-connection'].includes(incident.id));
    ...
    return { ...provider, status: severe ? 'incident' : active.length ? 'degraded' : 'healthy', source: 'derived from labelled log events', period: `last ${windowHours} uur` };
