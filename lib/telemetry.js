...
function normalizedPoint(point) {
  return {
    ...
    tab: point.tab || 'overview', title: point.title || 'Operational attention point', source: point.source || 'Dashboard',
...
export function deriveProviders(events = [], incidents = [], now = new Date(), windowHours = 24) {
  ...
  return [...providers.values()].map((provider) => {
    ...
    return { ...provider, status: severe ? 'incident' : active.length ? 'degraded' : 'healthy', source: 'derived from labelled log events', period: `last ${windowHours}h` };
