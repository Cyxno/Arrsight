...
export function buildAttentionItems(snapshot = {}, now = new Date()) {
  ...
  if (verifier.failed) points.push(normalizedPoint({ key:'verifier:failed', type:'verifier-failed', severity:'critical', tab:'downloads', title:'Verifier run failed', source:'Integrity verifier', advice:'Controleer de verifierlog en herstel de oorzaak voordat een nieuwe run start.', detail:'repairs', lastSeen:now.toISOString() }));
  else if (verifier.incomplete) points.push(normalizedPoint({ key:'verifier:incomplete', type:'verifier-incomplete', severity:'warning', tab:'downloads', title:'Verifier run remained incomplete', source:'Integrity verifier', advice:'Controleer waarom de oudere verifier-run niet netjes afsloot.', detail:'repairs', lastSeen:now.toISOString() }));
